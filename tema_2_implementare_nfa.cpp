...
class Graph{
    ...
    public:
    Graph(const std::vector<std::string> &Q, const std::vector<std::string> &lit, const std::string &initial_q,
            const std::unordered_set<std::string> &finalQ, const std::vector<std::string> &cuv,
            const std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> &map){
                this -> Q = Q;
                this -> lit = lit;
        ...
        void afisare(){
            ...
            for(auto [key_stare1, val]:map){
                for(auto [key_litera, stari2]:val){
                    for(auto stare2:stari2){
...
class Graph_DFA : public Graph{
    public:
        Graph_DFA(const std::vector<std::string> &Q, const std::vector<std::string> &lit, const std::string &initial_q,
            const std::unordered_set<std::string> &finalQ, const std::vector<std::string> &cuv,
            const std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> &map) : Graph(Q, lit, initial_q, finalQ, cuv, map){}
        void parcurgere(){
            ...
            for(std::string cuvant:cuv){
                ...
                while(pozitie < cuvant.size()){
                    ...
                    if (map[stare_curenta].find(str_poz) != map[stare_curenta].end()){
                        ...
                        cuvant.substr(pozitie+1)<<"|"<<map[stare_curenta][str_poz][0]<<'\n';
                        stare_curenta = map[stare_curenta][str_poz][0];
                        pozitie++;
                    }
                    else{
                        ended = false;
...
class Graph_NFA : public Graph{
    public:
        Graph_NFA(const std::vector<std::string> &Q, const std::vector<std::string> &lit, const std::string &initial_q,
            const std::unordered_set<std::string> &finalQ, const std::vector<std::string> &cuv,
            const std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> &map) : Graph(Q, lit, initial_q, finalQ, cuv, map){}
        void parcurgere(){
            std::unordered_set<std::string> multime_stari;
            std::unordered_set<std::string> temp_multime_stari;
            ...
            for(std::string cuvant:cuv){
                valid = false;
                multime_stari.clear();
                multime_stari.insert(initial_q);
                std::cout<<"Verificarea cuvantului: "<<cuvant;
                std::cout<<'\n';
                pozitie = 0;
                ...
                while(pozitie < cuvant.size()){
                    std::string str_poz = std::string(1, cuvant[pozitie]); // din char in string prin constructor
                    for(auto stare_curenta:multime_stari)
                        if (map[stare_curenta].find(str_poz) != map[stare_curenta].end())
                            for(auto stari:map[stare_curenta][str_poz]){
                                std::cout<<'\t';
                                std::cout<<cuvant.substr(pozitie)<<"|"<<stare_curenta<<" -> "<<
                                cuvant.substr(pozitie+1)<<"|"<<stari<<'\n';
                                temp_multime_stari.insert(stari);
                            }
                    multime_stari.clear();
                    for(auto stari: temp_multime_stari)
                            multime_stari.insert(stari);
                    temp_multime_stari.clear();
                    pozitie++;
                }
                for(auto stare_curenta:multime_stari){
                    if (finalQ.find(stare_curenta) != finalQ.end()){
                        std::cout<<'\t'<<"Cuvantul: "<<cuvant<<" este VALID";
                        valid = true;
                        break;
                    }
                ...
                if(!valid)
                    std::cout<<'\t'<<"Cuvantul: "<<cuvant<<" NU este VALID";
                std::cout<<'\n';
            }
        }
};
...
void initializare_DFA(){
    ...
    while(!fin.eof()){
        ...
        if(map[stare1_aux].find(litera_aux) != map[stare1_aux].end()){
            std::cout<<"Graful nu este DFA.";
            return;
        }
        map[stare1_aux][litera_aux].push_back(stare2_aux);
        check.clear();
    }
    fin.close();
...
void initializare_NFA(){
    ...
    fin.close();
    Graph_NFA Graph(Q, lit, initial_q, finalQ, cuv, map);
    Graph.afisare();
    Graph.parcurgere();
}
